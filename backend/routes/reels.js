...
function scorePostForUser(rawPost, tokenAffinity = {}) {
  ...
  // IMPORTANT: no random jitter so ordering is stable between requests
...
module.exports = function createPostsRouter(io, userSockets) {
  ...
  router.get('/feed', async (req, res) => {
    try {
      ...
      // EXTRA SAFETY: dedupe by _id so a post never appears twice in the feed
      const seenIds = new Set();
      const mergedUnique = merged.filter((p) => {
      ...
      const total = mergedUnique.length;
      const start = (page - 1) * pageSize;
      const end = start + pageSize;
      const pageRaw = mergedUnique.slice(start, end);
