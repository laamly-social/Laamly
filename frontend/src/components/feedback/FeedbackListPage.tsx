...
import { ArrowLeft, MessageCircle, AlertCircle, Lightbulb, MessageSquare, Clock, CheckCircle, Archive } from "lucide-react";
...
export default function FeedbackListPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "new" | "reviewing" | "resolved" | "archived">("all");
  ...
  const updateFeedbackStatus = async (id: string, newStatus: FeedbackItem["status"]) => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/feedback/${id}/status`, {
        method: "PATCH",
        headers: {
  ...
  const getStatusBadge = (status: string) => {
    const styles = {
      ...
      reviewing: "bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-200 border-yellow-300 dark:border-yellow-700",
      resolved: "bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200 border-green-300 dark:border-green-700",
      archived: "bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-700",
    };
    ...
    <div className="min-h-screen bg-bg dark:bg-bg-dark pb-20 md:pb-0">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          >
            <ArrowLeft size={24} className="text-text dark:text-text-dark" />
          </button>
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-sub dark:text-sub-dark">Loading feedback...</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            {filteredFeedback.length === 0 ? (
              <div className="text-center py-12">
                <MessageCircle size={48} className="mx-auto text-sub dark:text-sub-dark mb-4" />
                <p className="text-sub dark:text-sub-dark">No feedback found matching your filters.</p>
              </div>
            ) : (
              filteredFeedback.map((item) => (
                ...
                >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-text dark:text-text-dark">
                            {item.type === "bug" ? "🐛 Bug Report" : item.type === "feature" ? "💡 Feature Request" : "💬 General Feedback"}
                          </span>
                        ...
                        <p className="text-sm text-sub dark:text-sub-dark mt-1">
                          {item.userId?.profile?.name ? (
                            <span>
                              by <span className="font-medium">{item.userId.profile.name}</span>
                            </span>
                          ) : (
                            <span>Anonymous</span>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {["new", "reviewing", "resolved", "archived"].map((status) => (
                      <button
                        key={status}
                        onClick={() => updateFeedbackStatus(item._id, status as any)}
