...
export function Header({
   ...
   return (
      <>
         ...
         <nav
            className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#eeeeee] dark:bg-[#111111] border-t border-border dark:border-border-dark"
            style={{ backdropFilter: "blur(8px)" }}>
            <div className="flex items-center justify-around px-2 py-3">
               <a
                  href="/home"
                  className={`flex flex-col items-center justify-center p-2 rounded-lg transition ${location.pathname === "/home" ? "text-accent" : "text-text dark:text-text-dark"}`}>
                  <Home size={24} />
               </a>
               <a
                  href="/reels"
                  className={`flex flex-col items-center justify-center p-2 rounded-lg transition ${location.pathname === "/reels" ? "text-accent" : "text-text dark:text-text-dark"}`}>
                  <PlayTab size={24} />
               </a>
            ...
            <Podcast size={24} />
               ...
               {user && (
                  <a
                     href="/messages"
                     className={`flex items-center justify-center p-2 rounded-lg transition ${location.pathname === "/messages" ? "text-accent" : "text-text dark:text-text-dark"}`}>
                     <MessageSquare />
                  </a>
               )}
               {user && (
                  <a
                     href="/feedback"
                     className={`flex items-center justify-center p-2 rounded-lg transition ${location.pathname === "/feedback" ? "text-accent" : "text-text dark:text-text-dark"}`}>
               ...
               {user && (
                  <a
                     href="/notifications"
                     className={`flex items-center justify-center p-2 rounded-lg transition ${location.pathname === "/notifications" ? "text-accent" : "text-text dark:text-text-dark"}`}>
                     <NotificationBell />
                  </a>
               )}
               {user && user.name ? (
                  <button
                     onClick={() => openProfile(user.id)}
                     className={`flex flex-col items-center justify-center p-2 rounded-lg transition ${location.pathname.startsWith("/profile") ? "text-accent" : "text-text dark:text-text-dark"}`}>
                     <Avatar
                        src={user.avatar}
                        alt={user.name}
                        size="sm"
