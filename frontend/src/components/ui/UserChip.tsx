...
export default function UserChip({ avatar, handle, fullName, onClickName, variant = "default" }: { avatar: string; handle: string; fullName: string; onClickName?: () => void; variant?: "default" | "flat" }) {
  if (variant === "flat") {
    return (
      <div className="flex items-center gap-1.5 text-md">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-md text-sub dark:text-sub-dark">@{handle}</span>
        </div>
      </div>
    );
  }
