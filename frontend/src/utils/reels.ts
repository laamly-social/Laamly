...
export async function uploadReelVideo(file: File): Promise<string> {
   ...
   if ((data as any).status !== "ok")
      throw new Error((data as any).reason || "Upload failed");
...
export async function createReel(payload: {
   ...
   if (!res.ok)
      throw new Error((data as any)?.message || "Failed to create reel");
   return data as { message: string; reelId: string };
}

export async function fetchAllReels() {
   const res = await fetch(apiEndpoint("/reels/get-all"), {
      credentials: "include"
   });
   if (!res.ok) return [];
   const data = await res.json().catch(() => ({}));
...
export async function fetchReelById(id: string) {
   const res = await fetch(apiEndpoint(`/reels/${id}`), {
      credentials: "include"
   });
...
export async function createReelComment(reelId: string, text: string) {
   ...
   const data = await res.json().catch(() => ({}));
   if (!res.ok)
      throw new Error((data as any)?.message || "Failed to add comment");
   return data as {
      message: string;
      currentUser?: {
         id: string;
...
export async function editReelComment(
   ...
   const data = await res.json().catch(() => ({}));
   if (!res.ok)
      throw new Error((data as any)?.message || "Failed to edit comment");
...
export async function deleteReelComment(reelId: string, commentId: string) {
   ...
   const data = await res.json().catch(() => ({}));
   if (!res.ok)
      throw new Error((data as any)?.message || "Failed to delete comment");
   return data as { message: string };
}
...
export async function likeReelComment(reelId: string, commentId: string) {
   ...
   const data = await res.json().catch(() => ({}));
   if (!res.ok)
      throw new Error((data as any)?.message || "Failed to like comment");
